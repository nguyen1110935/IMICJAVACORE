...
public class Ex1 {
    public static void main(String[] arg) {
        ...
        double[] list_unorderd = user_input(scan.nextInt());
        double[] list_ordered;
        int index;
        double f_value;
        //Ex 5
        ...
        if (index == -1){
            ...
            if (index != 0) {
                while (true) {
                    if (index + variant == -1) break;
                    if (f_value == list_ordered[index + variant]) {
                        System.out.println("Detected in index= " + addExact(index, variant));
                        variant--;
                    } else break;
            ...
            if (index != list_ordered.length-1) {
                while (true) {
                    if (index + variant == list_ordered.length) break;
                    if (f_value == list_ordered[index + variant]) {
                        System.out.println("Detected in index= " + addExact(index, variant));
                        variant++;
                    } else break;
